...
function createPasswordEntry(name, password) {
  ...
  passEntry.innerHTML = `
    <div style="display: flex; align-items: center; gap: 10px;">
      <p style="margin: 0"><strong>${name}:</strong> <span class="password-text" data-password="${password}">${password}</span></p>
    </div>
    <div style="display: flex; gap: 10px;">
      <button onclick="copyPassword('${password}', this)" style="background: none; border: none; cursor: pointer;">
...
function copyPassword(password, element) {
  const isHidden = element.closest('.password-entry').querySelector('.password-text').classList.contains('hidden-password');
  if (isHidden) {
    navigator.clipboard.writeText('********');
    showNotification('Hidden passwords cannot be copied for security', 'warning');
  } else {
...
const GENERATE_COOLDOWN = 10000; // 10 second cooldown

generateBtn.addEventListener('click', () => {
  const now = Date.now();
  if (now - lastGenerateTime < GENERATE_COOLDOWN) {
    showNotification('Please wait before generating another password', 'warning');
    return;
  }
  ...
  if (!length || length < 5) {
    showError(lengthInput, 'Length must be at least 5 characters');
    hasError = true;
  }
  ...
  savePasswords();
  showNotification('New password generated successfully');
...
toggleBtn.addEventListener('click', () => {
  ...
  passwords.forEach(pass => {
    const originalPassword = pass.getAttribute('data-password');
    if (passwordsVisible) {
      pass.textContent = originalPassword;
      pass.classList.remove('hidden-password');
      toggleBtn.innerHTML = '<i class="fas fa-eye"></i>  Hide Passwords';
    } else {
      pass.textContent = '*'.repeat(originalPassword.length);
      pass.classList.add('hidden-password');
      toggleBtn.innerHTML = '<i class="fas fa-eye-slash"></i> Show Passwords';
    }
  });
});